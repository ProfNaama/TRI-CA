...
function readLocalCodes() {
    if (!config.localCodesFile) {
        return null;
    }

    const fileText = fs.readFileSync(config.localCodesFile, 'utf8').trim();
    if (!fileText) {
        return [];
    }
    ...
    if (!Array.isArray(records)) {
        throw new Error('LOCAL_CODES_FILE must contain either a JSON array or an object with a "codes" array');
    }

    return records
        .map(record => {
            if (typeof record === 'string') {
                return {code: record.trim(), expid: null, completed: null};
            }
            ...
            if (record && typeof record === 'object') {
                return {
                    ...
                    expid: record.expid ? String(record.expid).trim() : null,
...
function writeLocalCodes(records) {
    if (!config.localCodesFile) {
        return false;
    }

    const tempFile = `${config.localCodesFile}.tmp`;
    fs.writeFileSync(tempFile, JSON.stringify(records, null, 2) + "\n");
    fs.renameSync(tempFile, config.localCodesFile);
    return true;
}

function getLocalCodeRecord(code, records) {
    if (!records) {
        return null;
    }

    const normalizedCode = String(code).trim();
    const matchingRecords = records.filter(record => record.code === normalizedCode);
    if (matchingRecords.length === 0) {
        return null;
    }

    const experimentId = config.experimentId ? String(config.experimentId).trim() : null;
    if (experimentId) {
        return matchingRecords.find(record => record.expid === experimentId)
            || matchingRecords.find(record => !record.expid)
            || null;
    }

    if (matchingRecords.length > 1) {
        console.log("Error: ambiguous LOCAL_CODES_FILE match for code " + normalizedCode + ". Set EXPERIMENT_ID or use unique codes.");
        return null;
    }
...
async function isCodeValid(code) {
    if (code) {
        ...
        if (config.localCodesFile) {
            try {
                const localCodes = readLocalCodes();
                const localCodeRecord = getLocalCodeRecord(code, localCodes);
                if (localCodeRecord) {
                    return !localCodeRecord.completed;
                }
            } catch (error) {
                console.log("Error: " + error);
                return false;
...
async function setCodeCompleted(code, obj) {
    ...
    if (config.localCodesFile) {
        try {
            ...
            const localCodeRecord = getLocalCodeRecord(code, localCodes);
            if (localCodeRecord) {
                localCodeRecord.completed = obj || true;
                return writeLocalCodes(localCodes);
            }
        } catch (error) {
            console.log("Error: " + error);
            return false;
